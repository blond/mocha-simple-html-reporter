...
// If title contains http(s) appends clickable
// link to code for easy in-browser testing of
// internal API's
const appendURL = title => {
    const match = /^http.?:\/\/(?:[^/]+)(.*)/.exec(title)
    return match ? `<a href="${ title }">${ title }</a>\n\n<a href="${ match[1] }"">${ match[1] }</a>\n\n` : ''
}
...
module.exports = function (runner, options) {
    ...
    runner.on('pass', test => {
        ...
        result += `<pre>${ appendURL(title) }<code>${code}</code></pre>`;
        result += '</li>';
    });
